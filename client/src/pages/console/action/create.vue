...
export default {
    ...
    methods: {
        async submit(data){
            ...
            try{
                data                    = await this.$request.post("/client/action/create", {
                    user_id             : this.$route.query.user_id,
                    ...this.form
                });
                let action              = data.data.action;
                this.$EventBus.emit('organization-action-create', { action });
                await Time.delay(1)
                this.submit_ing         = false;
                this.link({path:'/console/action/details',query:{action_id:action._id,user_id:action.user_id}},'replace')
            } catch (e){
                this.submit_ing         = false;
                console.error(e)
            }
