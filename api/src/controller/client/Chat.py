...
class Chat(OrganizationController):
    ...
    async def create(self):
        ...
        for u in users:
            if u.role == 'assistant' and u.settings['opening_speech'].strip():
                await u.opening_answer(chat)
        await self.organization.event_publish('chat-create',{'chat':chat})
        return {'chat':chat},
    ...
    async def search(self):
        ...
        task_id                         = post.get('task_id','')         # 创建者id
    ...
    async def destroy(self):
        ...
        await self.organization.event_publish('chat-destroy',{'chat':chat})
        return {},'删除成功'
    ...
    async def leave(self):
        ...
        if user_id in user_ids:
            ...
            await self.organization.event_publish('chat-user-leave',{'chat':chat,'user':{'_id':user_id}})
        ...
        await self.organization.event_publish('chat-update',{'chat':chat})
    ...
    async def invite(self):
        ...
        for i,user in enumerate(users):
            ...
            if user._id not in chat.user_ids:
                ...
                await self.organization.event_publish('chat-user-join',{'chat':chat,'user':user})
        ...
        for u in users:
            if u.role == 'assistant' and u.settings['opening_speech'].strip():
                message                 = await u.opening_answer(chat)
                messages.append(message)
                await self.organization.event_publish('chat-message-stream',message)
        await self.organization.event_publish('chat-update',{'chat':chat})
    ...
    async def messageDel(self):
        ...
        if not message_id:
            raise CodeError('请传入消息ID')
        message                         = await EChatMessage.afrom(_id=message_id,_source=['_id','chat_id','status','files'])
        if message:
            await message.destroy()
            await self.organization.event_publish('chat-message-remove',message)
        return {},'删除成功'

    async def up_avatar(self):
        post                            = await self.get_post()
        chat_id                         = post.get('chat_id','')
        chat                            = await EChat.afrom(_id=chat_id)
        data                            = {}
        fs                              = request.files['avatar']
        efile                           = await EFile.upload(fs, self.user, organization=self.organization, location='public',refresh=False)
        olfie                           = await EFile.afrom(url=chat.avatar)
        await chat.upset(avatar=efile.url)
        if olfie:
            await olfie.destroy(refresh=False)
        return {'chat':chat},'上传成功'